export function installLiveBindings(ctx) {
  ...
  const applyLiveSpeakerIdentityAlias = (...args) => ctx.api.applyLiveSpeakerIdentityAlias(...args);
  ...
  function connect() {
    ...
    ctx.owners.capture.es.addEventListener("live_speaker_identity_alias", e => applyLiveSpeakerIdentityAlias(JSON.parse(e.data)));
  ...
  ctx.activators.push(() => {
    ...
    const undoLastCorrection = async () => {
      try {
        ...
        ctx.owners.transcript.hasUndoableCorrection = Boolean(result.can_undo);
        syncCorrectionUndoState(ctx.owners.transcript.hasUndoableCorrection);
    ...
    undoCorrectionButton.addEventListener("click", undoLastCorrection);
    if (speakerUndoCorrection) speakerUndoCorrection.addEventListener("click", undoLastCorrection);
    ...
    newRunSessionButton.addEventListener("click", () => {
      resetLiveSpeakerPresentation("new-session");
      createNewSession();
    });
