"""Test short-only identity fusion at causally detected speaker boundaries."""
...
def main() -> int:
    ...
    variants = (
        (-1.0, 0.03, 0.035, 0.04, 0.045, 0.05, 0.055, 0.06, 0.0625, 0.065, 0.07, 0.075)
        if args.refine else
        (-1.0, -0.10, -0.05, 0.0, 0.03, 0.05, 0.06, 0.08, 0.10, 0.15, 0.20, 0.25, 0.30)
    )
    ...
    for threshold in variants:
        config = replace(base, boundary_short_only_max_continuity=threshold)
        ...
        row = {
            "boundary_short_only_max_continuity": threshold,
    ...
    _atomic(run_dir / "champion.json", {
        "status": "CACHE_BOUNDARY_SHORT_ONLY_WINNER_PENDING_FRESH_LIVE",
        ...
        "hypothesis": (
            "At a causal voice discontinuity, suppress the slower contaminated window for "
            "one decision and score identity from the short window only."
        ),
        **best,
