"""Tune a causal short/long differential speaker-change detector."""
...
def main() -> int:
    ...
    if args.refine:
        candidate_values = (-0.50, -0.40, -0.30, -0.25, -0.20, -0.175, -0.15, -0.125, -0.10)
        incumbent_values = (0.10, 0.125, 0.15, 0.175, 0.20, 0.225, 0.25, 0.30, 0.40)
    else:
        candidate_values = incumbent_values = (
            -0.15, -0.10, -0.05, -0.025, 0.0, 0.025, 0.05, 0.075, 0.10, 0.15
        )
    variants = [(-2.0, -2.0), *itertools.product(candidate_values, incumbent_values)]
    gate_root = args.gate_root.resolve()
    run_dir = args.run_dir.resolve()
    run_dir.mkdir(parents=True, exist_ok=True)
    rows: list[dict[str, Any]] = []
    for candidate_gain, incumbent_loss in variants:
        config = replace(
            base,
            short_long_differential_candidate_gain=candidate_gain,
            short_long_differential_incumbent_loss=incumbent_loss,
        )
        ...
        _atomic(run_dir / "progress.json", {
            ...
            "active": [candidate_gain, incumbent_loss],
        })
    best = max(rows, key=lambda item: float(item["aggregate"]["primary_score"]))
    _atomic(run_dir / "trials.json", rows)
    _atomic(run_dir / "champion.json", {
        "status": "CACHE_DIFFERENTIAL_CROSSOVER_WINNER_PENDING_FRESH_LIVE",
