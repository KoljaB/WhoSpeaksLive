"""Tune a separate causal RMS release threshold and speech-duration hysteresis."""
...
def main() -> int:
    ...
    probe_threshold = float(source["vad_speech_rms_threshold"])
    clear_window = float(source["live_speaker_clear_window_seconds"])
    probe_min_speech = float(source["live_speaker_probe_min_speech_seconds"])
    release_thresholds = (0.0014, 0.0017, 0.0020, 0.0023, 0.0026, 0.0030, 0.0035)
    release_min_speech_values = (0.03, 0.06, 0.09, 0.12, 0.15, 0.18, 0.21, 0.24, 0.30)
    release_counts = (1, 2, 3, 4)
    total = len(release_thresholds) * len(release_min_speech_values) * len(release_counts)
    rows: list[dict[str, Any]] = []
    for release_threshold in release_thresholds:
        for release_min_speech in release_min_speech_values:
            variant = f"release_{release_threshold:.4f}_speech_{release_min_speech:.2f}"
            ...
            for video_id in videos:
                ...
                build_video(
                    ...
                    min_speech_seconds=probe_min_speech, release_every_tick=True,
                    release_threshold=release_threshold,
                    release_min_speech_seconds=release_min_speech,
                )
            for release_count in release_counts:
                ...
                row = {
                    ...
                    "live_speaker_probe_min_speech_seconds": probe_min_speech,
                    "live_speaker_release_min_speech_seconds": release_min_speech,
    ...
    _atomic(run_dir / "champion.json", {
        "status": "CACHE_RMS_HYSTERESIS_WINNER_PENDING_FRESH_LIVE",
