...
tools/realtime_speakerdiarize.py --validate-cunk-realtime-replay, so the result
is directly comparable to the ElevenLabs canonical baseline.

For a browser UI with synced local video playback, use
tools/youtube_local_filefeed_gui.py.
"""
...
from realtime_speakerdiarize import extract_youtube_video_id  # noqa: E402


DEFAULT_URL = "https://www.youtube.com/watch?v=JWS-qfR6K3w"
DEFAULT_WORK_DIR = TOOLS / ".local_filefeed_media"
DEFAULT_OUTPUT_DIR = TOOLS / ".realtime_speakerdiarize_validation"
DEFAULT_CANONICAL = ROOT / "output_elevenlabs_cunk" / "cunk_on_earth_clip.canonical_diarization.json"
DEFAULT_EMBEDDING_PYTHON = ROOT / ".venv-voice-embeddings" / "Scripts" / "python.exe"
...
KNOWN_YT_DLP_EXES = [
    ...
    ROOT / ".venvs" / "install-matrix" / "all" / "Scripts" / "yt-dlp.exe",
    Path("D:/Projekte/STT/RealtimeSTT/RealtimeSTT/.venvs/install-matrix/all/Scripts/yt-dlp.exe"),
]
...
def build_replay_command(
    ...
    command = [
        str(args.python),
        str(ROOT / "tools" / "realtime_speakerdiarize.py"),
