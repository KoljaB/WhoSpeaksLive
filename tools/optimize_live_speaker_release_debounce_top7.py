"""Tune consecutive dense release signals for a precomputed live gate."""
...
def main() -> int:
    ...
    parser.add_argument("--clear-window-seconds", type=float, required=True)
    ...
    variants = range(1, 9)
    for release_count in variants:
        config = replace(base, silence_release_count=release_count)
        ...
        row = {
            "live_speaker_clear_window_seconds": args.clear_window_seconds,
        ...
        _atomic(run_dir / "progress.json", {
            ...
            "total_candidate_count": len(variants), "best_score": best["aggregate"]["primary_score"],
            "active_release_count": release_count,
        })
    best = max(rows, key=lambda item: float(item["aggregate"]["primary_score"]))
    _atomic(run_dir / "trials.json", rows)
    _atomic(run_dir / "champion.json", {
        "status": "CACHE_RELEASE_DEBOUNCE_WINNER_PENDING_FRESH_LIVE",
