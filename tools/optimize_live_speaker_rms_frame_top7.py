"""Tune RMS frame geometry around the promoted dense-release gate."""
...
def main() -> int:
    ...
    frame_values = (0.010, 0.015, 0.020, 0.025, 0.030, 0.040, 0.050, 0.060, 0.080, 0.100)
    release_counts = (1, 2, 3, 4)
    run_dir = args.run_dir.resolve()
    run_dir.mkdir(parents=True, exist_ok=True)
    rows: list[dict[str, Any]] = []
    for frame_seconds in frame_values:
        variant = f"frame_{frame_seconds:.3f}"
        ...
        for video_id in videos:
            ...
            build_video(
                ...
                cadence_seconds=0.75, frame_seconds=frame_seconds, threshold=threshold,
        ...
        for release_count in release_counts:
            ...
            _atomic(run_dir / "progress.json", {
                ...
                "total_candidate_count": len(frame_values) * len(release_counts),
                "best_score": best["aggregate"]["primary_score"], "active_variant": variant,
            })
    best = max(rows, key=lambda item: float(item["aggregate"]["primary_score"]))
    _atomic(run_dir / "trials.json", rows)
    _atomic(run_dir / "champion.json", {
        "status": "CACHE_RMS_FRAME_WINNER_PENDING_FRESH_LIVE",
