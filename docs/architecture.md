...
Persistent People are a separate layer beside meeting-local Speaker memory. The versioned People library stores `Person → VoiceSample → EmbeddingRepresentation`, with independent manual and confirmed-meeting source policy. Matching is suggestion-first, filters by provider and dimension, and uses strongest evidence plus capped independent corroboration rather than an unbounded gallery maximum. Linking identity is separate from enrolling voice evidence: when a robust learning candidate is unavailable, the link succeeds without storing a fallback centroid. Safe checkpoints update only the current session's meeting sample; automatic samples are capped per provider, while manual and explicitly user-confirmed evidence keeps its trust level. Deleted meeting sources leave suppression markers so saved-session recalculation cannot silently recreate them. Saved-session link/unlink writes follow SessionStore-then-People lock order and use a durable, replayed transaction intent for interruption recovery.
...
Most user actions in the browser call JSON API routes, such as Speaker rename, saved-session correction, Person linking, Voice-sample management, recognition-roster selection, and legacy Speaker-group import/export. Browser payloads omit vectors and absolute local paths but still contain sensitive identity and participation metadata. The built-in server is a trusted-operator interface, not an authenticated multi-user API; see [Security And Data Privacy](security-and-data-privacy.md).
